        ...
        <div className="w-1/2 h-40 bg-slate-400 flex items-center">
          ...
          <button className="w-24 h-12 bg-gray-50 ml-3 mb-20 rounded-2xl text-slate-400 text-xl outline-none p-2 shrink-0">
            ...
            <div className="absolute flex items-center ml-64 gap-x-2">
              <input type="checkbox" defaultChecked={numcheck} id="numberInput" onChange={()=>{(prev)=!prev}} />
               <label htmlFor="numberInput" className="text-emerald-100">Numbers</label>
            </div>
            <div className="absolute flex items-center ml-96 gap-x-2">
              <input type="checkbox" defaultChecked={charcheck} id="charInput" onChange={()=>{(prev)=!prev}} />  {/* makes charcheck true when false and false when true whenever you click in check button. */}
               <label htmlFor="charInput"className="text-emerald-100">Characteres</label>
            </div>
          </div>
        </div>
    </>
